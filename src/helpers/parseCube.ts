...
const converseToPaperType = (cubeOutputDataDebug: Uint8Array) => {
  const array = new Uint8Array(55);
  let num = 0;
  const array2 = new Uint8Array(8);
  const array3 = new Uint8Array(8);
  const array4 = new Uint8Array(12);
  const array5 = new Uint8Array(12);
...
const cubeDataMixDecode = (mixData: DataView) => {
  ...
  for (let b3 = 0; b3 < 19; b3 += 1) {
    ...
    const array3 = array;
    const b4 = b3;
    array3[b4] -= array2[b + b3];
    const array4 = array;
    const b5 = b3;
    array4[b5] -= array2[b2 + b3];
  }
  return array;
};

const parseCube = (bytes: DataView) => {
  const mixerDataDecoded = cubeDataMixDecode(bytes);
  // if it returned a dataview, it means it was not decoded maybe?
  let paperTypeCube: Uint8Array | null = null;
  if (mixerDataDecoded instanceof DataView) {
    paperTypeCube = converseToPaperType(new Uint8Array(mixerDataDecoded.buffer));
  } else {
